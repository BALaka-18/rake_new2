...
cluster_model = KMeans(n_clusters=2, max_iter=1000, random_state=True,
                       n_init=50).fit(vectors.vectors.astype("double"))

vectors.similar_by_vector(cluster_model.cluster_centers_[
                          1], topn=10, restrict_vocab=None)
...
keywords[['keywords', 'sentiment_coeff']].to_csv(
    'sent_dictionary.csv', index=False)
