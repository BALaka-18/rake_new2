...
const ExampleForm = () => {
  ...
  const radio1 = [
    { id: enums.KEEP_HTML, value: "Keep Html" },
    { id: enums.DONT_KEEP, value: "Don't Keep" },
  ];
  const radio2 = [
    { id: enums.SHOW_WORDS, value: "Show Top 5 most Frequent Words" },
    { id: enums.DONT_SHOW, value: "Don't Show" },
  ];
  const selectOptions = [
  ...
  const getBody = () => {
    let otherOption;
    if (dropdownSelect == null) otherOption = "null";
    else if (dropdownSelect === enums.GET_KEYWORDS_ONLY) otherOption = "kwrd";
    else if (dropdownSelect === enums.GET_KEYWORDS_SCORES)
      otherOption = "kwrd_score";
    else if (dropdownSelect === enums.GET_TOP_5_KEYWORDS) otherOption = "top5";
    const body = {
      text: text,
    ...
    <MDBContainer className="example-form">
      ...
      <MDBRow className="justify-content-center">
        <Select
          ...
          onChange={(dropdown) => setDropdownSelect(dropdown.value)}
        />
      </MDBRow>
      <MDBRow className="mt-2 justify-content-center">
        <MDBBtn gradient="purple" onClick={() => extract()}>
          Extract
        </MDBBtn>
      </MDBRow>
      <MDBBox className="output">
        {output.top5Words ? (
          <Fragment>
            ...
            <br />
            {Object.keys(output.top5Words).map((key, index) => (
              <p>
                {key} = {output.top5Words[key]}
              </p>
            ))}
          </Fragment>
        ) : (
